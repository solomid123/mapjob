"""
Supabase Database Initialization & Migration Script for MapJob & Interview Copilot.
Connects directly to Postgres, creates all required tables, indexes, RLS policies,
and storage buckets for resumes, interview artifacts, and job attachments.
"""
...
-- 3. JOB APPLICATIONS TABLE (Auto-Apply Automation)
...
-- 7. INTERVIEW SCREEN ANALYSES TABLE (Vision Frames & OCR Analysis)
...
def init_supabase():
    ...
    print("Verified public tables:", tables)
    
    # Verify buckets
    cur.execute("SELECT id, name, public FROM storage.buckets;")
    buckets = cur.fetchall()
    print("Verified storage buckets:", buckets)
    
    cur.close()
    conn.close()
    print("Supabase database setup is 100% COMPLETE!")
